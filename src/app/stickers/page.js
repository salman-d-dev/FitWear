...
const getStickers = async () => {
  ...
  for (const title in stickers) {
    stickers[title].color = stickers[title].color.filter((color) =>
      products.some(
        (product) =>
          product.title === title &&
          product.color === color &&
    ...
    stickers[title].size = stickers[title].size.filter((size) =>
      products.some(
        (product) =>
          product.title === title &&
          product.size === size &&
                          ...
                          <div className="my-1">
                            {products[key].color.map((col, index) => (
                              <button
                                ...
                                className="ml-1 rounded-full w-5 h-5 focus:outline-none "
                              ></button>
                            ))}
                          </div>
                        )}
