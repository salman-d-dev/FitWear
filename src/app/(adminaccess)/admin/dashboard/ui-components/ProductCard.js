...
export default function ProductCard({title, slug, description, category, price, availableQty, size,color, img,handleRemoveProduct}) {
  return (
    <div className='bg-slate-100 flex items-center justify-center flex-col rounded-2xl p-2 shadow-lg text-center my-3'>
        <h2 className='font-light'>{category}</h2>
        <h1 className='font-bold'>{title}</h1>
        <h2 className='font-light text-sm'>Slug:{slug}</h2>
        <img className="w-full rounded-md my-2" src={img? img :""} alt="Product.jpg"/>
        <h3>₹{price}</h3>
        ...
        <p className="text-sm italic break-words w-fit">{description.length>=20? description.slice(0,20) +"..." : description}</p>
